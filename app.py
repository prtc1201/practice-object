...
def handle_message(event):
    ...
    with ApiClient(configuration) as api_client:
        ...
        #當使用者輸入的文字作為faq的key，就將value作為bot_msg
        if user_msg in faq:
        ...
        #當使用者的文字存在幣別清單中
            ...
            completion = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "你是一個充滿人生智慧的人，會幽默又包容的用繁體中文回答使用者問題"},
                    {
                        "role": "user",
                        "content": user_msg
                    }
        ...
        line_bot_api.reply_message_with_http_info(
            ReplyMessageRequest(
                ...
                messages=[
                    # 機器人要回應給使用者的內容放在這個串列中
                    # Line文字訊息只有其工程師知道確切語言跟類別，所以要用TextMessage包裝，才能產生Line能接受的文字格式
                    #已讀表示Line伺服器收到使用者傳來的訊息
                    TextMessage(text="請輸入menu觀看選單"),
                    bot_msg
                ]
            )
        )
